...
export default class VirtualList extends React.Component {
    ...
    resize() {
        ...
        const itemSize = (r => !r ? 1 : ((h, i) => {
            r.style.height = null;
            i = r.clientHeight;
            r.style.height = h;
            return i
        })(r.style.height))(fix.children[0].children[0]);
        const height = (f => ((d, i) => {
            f.style.display = "none";
            i = f.parentElement.clientHeight;
            f.style.display = d;
            return i
        })(f.style.display))(fix);
    ...
    render() {
        ...
        return (
            <FlexPullToRefresh canRefresh={() => this.listRef.current._outerRef.scrollTop === 0}
                               onRefresh={this.props.onPull}>
                <FixedSizeList ref={this.listRef} width="100%" height={height} itemSize={itemSize}
